...
interface BulkMenuItem {
    title: string
    tooltip: string
    action: (overview: DocumentOverview) => void
    disabled: (overview: DocumentOverview) => boolean
    order: number
    id?: string
}
...
interface MenuItem {
    ...
    action?: (overview: DocumentOverview) => void
    icon?: string
    input?: (overview: DocumentOverview, text: string) => void
    order: number
...
export interface MenuModel {
    content: (MenuItem | BulkMenuItem)[]
}

export const bulkMenuModel = (): MenuModel => ({
    content: [
        {
            ...
            action: overview => {
                ...
                if (ownIds.length) {
                    ...
                    const dialog = new DocumentAccessRightsDialog(
                        ids,
                        overview.contacts as any,
                        ((memberDetails: Record<string, unknown>) =>
                            overview.contacts.push(memberDetails as any)) as any,
                        e2ee,
                        "",
                        null as any,
...
export const menuModel = (): MenuModel => ({
    content: [
        ...
        },
        {
            ...
            keys: "Alt-f",
            action: overview => {
                const dialog = new NewFolderDialog((folderName: string) => {
